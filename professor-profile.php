...
$query = "
    SELECT 
        ...
        professor.cv_path
    FROM users
    LEFT JOIN professor ON users.id = professor.user_id
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // تحديث professor
    mysqli_query($conn, "UPDATE professor 
                         SET name='$name', email='$email', department='$dept', university='$univ' 
                         WHERE id=$user_id");

    // التحقق إذا يوجد سجل في professor
    $prof_check = mysqli_query($conn, "SELECT * FROM professor WHERE user_id=$user_id");
    if (mysqli_num_rows($prof_check) > 0) {
        // تحديث
        mysqli_query($conn, "UPDATE professor 
                             SET cv_path='$cv_path' 
        ...
        // إدراج جديد
        mysqli_query($conn, "INSERT INTO professor (user_id, cv_path) 
                             VALUES ($user_id,  '$cv_path')");
    }

    header("Location: professort_profile.php");
    exit;
}
/*session_start();
include 'index.php';
                    ...
                    <input type="text" name="university" value="<?= htmlspecialchars($professor['university'] ?? '') ?>">
                            ...
                            <!-- عرض CV الحالي -->
                             ...
                             <?php if (!empty($professor['cv_path'])): ?>
                               <label><a href="<?= htmlspecialchars($professor['cv_path']) ?>" target="_blank">CV</a></label>
                                <?php else: ?>
