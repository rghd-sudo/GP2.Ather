...
function updateStatus(requestId, action) {
  ...
  formData.append('action', action); // ✅ عدلنا هنا
  ...
  fetch('', { method: 'POST', body: formData })
    ...
    .then(status => {
      ...
      if (status === 'accepted') {
        container.innerHTML = '<div class="status-box accepted">Accepted</div>';
      } else if (status === 'rejected') {
        container.innerHTML = '<div class="status-box rejected">Rejected</div>';
      }
    })
