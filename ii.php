...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email && $code) {
        ...
        if ($user) {
            ...
            $_SESSION['message'] = "✅ تم تفعيل الحساب بنجاح!";
...
// إنشاء جدول المستخدمين (مع verify_code و verified)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $email && $paasword && $department && $National_ID && $university) {
        if ($paasword !== $confirm_password) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (name, email, paasword, department, National_ID, university, verify_code, verified) VALUES (?, ?, ?, ?, ?, ?, ?, 0)");
            $stmt->bind_param("sssssss", $name, $email, $hashed, $department, $National_ID, $university, $verify_code);

            if ($stmt->execute()) {
                $user_id = $stmt->insert_id;

                // إدخال بيانات الخريج في graduates
                ...
                // إرسال الإيميل (في XAMPP قد لا يعمل mail، لذلك نعرض الرمز مؤقتاً)
                $subject = "رمز تفعيل حسابك في موقعنا";
                $message = "مرحباً $name،\n\nرمز التحقق الخاص بك هو: $verify_code\n\nأدخليه في صفحة التحقق لإتمام التسجيل.";
                $headers = "From: no-reply@yourdomain.com";

                if (@mail($email, $subject, $message, $headers)) {
                    $_SESSION['message'] = "✅ تم التسجيل بنجاح! تم إرسال رمز التحقق إلى بريدك الإلكتروني.";
                } else {
                    // عرض الرمز على الشاشة فقط أثناء التطوير
                    $_SESSION['message'] = "✅ تم التسجيل بنجاح! (رمز التحقق للتجربة المحلية): $verify_code";
                }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = $_POST['name'];
    $email = $_POST['email'];
    $paasword = password_hash($_POST['paasword'], PASSWORD_DEFAULT);
    $verify_code = rand(100000, 999999); // رمز تحقق مكوّن من 6 أرقام

    // حفظ المستخدم مع حالة غير مفعلة
    $sql = "INSERT INTO users (name, email, paasword, verify_code, verified) 
            VALUES ('$name', '$email', '$paasword', '$verify_code', 0)";
    ...
    echo "تم التسجيل بنجاح! تم إرسال رمز التحقق إلى بريدك الإلكتروني.";
}
