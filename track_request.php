...
$requests = [];
$sql = "SELECT id, COALESCE(purpose,'') AS purpose, COALESCE(status,'') AS current_status, created_at 
...
.menu-item { display:flex; align-items:center; padding:12px 20px; color:#333; text-decoration:none; transition:0.3s; }
...
.top-bar { position:fixed; top:0; right:0; left:230px; height:60px; display:flex; justify-content:flex-end; align-items:center; padding:0 20px; transition:left 0.3s; z-index:10; }
